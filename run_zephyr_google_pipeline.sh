...
BRANCH_NAME="${ZEPHYR_BRANCH_NAME:-${ZEPHYR_GSHEET_BRANCH_NAME:-}}"
SPREADSHEET_ID="${ZEPHYR_GSHEET_SPREADSHEET_ID:-${GOOGLE_SPREADSHEET_ID:-}}"
SPREADSHEET_TITLE="${ZEPHYR_GSHEET_SPREADSHEET_TITLE:-${GOOGLE_SPREADSHEET_TITLE:-Zephyr Daily Execution Sheet}}"
CONFIG_SHEET="${ZEPHYR_GSHEET_CONFIG_SHEET:-${GOOGLE_CONFIG_SHEET:-Config}}"
RUN_SHEET="${ZEPHYR_GSHEET_RUN_SHEET:-${GOOGLE_RUN_SHEET:-Run}}"
...
TESTCASE_ENDPOINT_TEMPLATE="${ZEPHYR_GSHEET_TESTCASE_ENDPOINT_TEMPLATE:-${ZEPHYR_TESTCASE_ENDPOINT_TEMPLATE:-rest/tests/1.0/testrun/{cycle_id}/testcase/search}}"
SYNTHETIC_CYCLE_IDS="${ZEPHYR_GSHEET_SYNTHETIC_CYCLE_IDS:-${ZEPHYR_SYNTHETIC_CYCLE_IDS:-true}}"
FROM_DATE="${ZEPHYR_FROM_DATE:-}"
TO_DATE="${ZEPHYR_TO_DATE:-}"
FOLDER_PARENT_ID="${ZEPHYR_GSHEET_FOLDER_PARENT_ID:-${ZEPHYR_FOLDER_PARENT_ID:-}}"
FOLDER_PATH_REGEX="${ZEPHYR_GSHEET_FOLDER_PATH_REGEX:-${ZEPHYR_FOLDER_PATH_REGEX:-}}"
STATUS_PASS_NAME="${ZEPHYR_GSHEET_STATUS_PASS_NAME:-${ZEPHYR_STATUS_PASS_NAME:-Pass}}"
STATUS_FAIL_NAME="${ZEPHYR_GSHEET_STATUS_FAIL_NAME:-${ZEPHYR_STATUS_FAIL_NAME:-Fail}}"

UPDATE_ENDPOINT_TEMPLATE="${ZEPHYR_GSHEET_UPDATE_ENDPOINT_TEMPLATE:-${ZEPHYR_UPDATE_ENDPOINT_TEMPLATE:-rest/tests/1.0/testresult/{test_result_id}}}"
UPDATE_METHOD="${ZEPHYR_GSHEET_UPDATE_METHOD:-${ZEPHYR_UPDATE_METHOD:-PUT}}"
UPDATE_STATUS_ID_FIELD="${ZEPHYR_GSHEET_UPDATE_STATUS_ID_FIELD:-${ZEPHYR_UPDATE_STATUS_ID_FIELD:-testResultStatusId}}"
UPDATE_COMMENT_FIELD="${ZEPHYR_GSHEET_UPDATE_COMMENT_FIELD:-${ZEPHYR_UPDATE_COMMENT_FIELD:-comment}}"
UPDATE_EXTRA_BODY_JSON="${ZEPHYR_GSHEET_UPDATE_EXTRA_BODY_JSON:-${ZEPHYR_UPDATE_EXTRA_BODY_JSON:-}}"
...
if [[ -z "$BRANCH_NAME" ]]; then
  if [[ -n "${ZEPHYR_GSHEET_BRANCH_NAME_TEMPLATE:-}" ]]; then
    BRANCH_NAME="$(date +"$ZEPHYR_GSHEET_BRANCH_NAME_TEMPLATE")"
  fi
fi
if [[ -z "$BRANCH_NAME" ]]; then
...
if [[ "$MODE" == "generate" ]]; then
  ...
  if [[ -z "$SPREADSHEET_ID" ]]; then
    echo "Set ZEPHYR_GSHEET_SPREADSHEET_ID (or GOOGLE_SPREADSHEET_ID) for sync mode." >&2
    exit 1
  fi
  cmd+=(
    "sync-sheet"
